...
const createHTML = () => {
  ...
  //left
  const divLeft = createAndAppend('div', divRoot, {
    id: 'divLeft'
  });
  const h1 = createAndAppend('h1', divLeft, {
    text: 'FooCafe Repository'
  });
  const ul = createAndAppend('ul', divLeft, {
    id: 'list'
  });
  //right
  const divRight = createAndAppend('div', divRoot, {
    id: 'contributors',
  });
}
...
const createContributor = repo => {
  fetch(repo.contributors_url)
    ...
    .then(data => {
      ...
      const div = createAndAppend('div', contributors, {
        id: 'contributorsItem'
      });
      createAndAppend('h1', div, {
        id: 'item',
        text: 'Contributions'
      });
      ...
      for (const i in data) {
        ...
        const badge = createAndAppend('div', li, {
          class: 'contributorsBadge',
...
const url = main('https://api.github.com/orgs/HackYourFuture/repos?per_page=30');
console.log(url);