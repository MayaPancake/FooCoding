...
function fetchPokemonData() {
  ...
  const pokemonLists = JSON.parse(fetchPokemonData());
  const spritesLists = Object.values(pokemonLists.sprites);
    const imagesLists = spritesLists.filter(i => i != null);

const makeHtmlLists = imagesLists.map(i => {
  let myImage;
  pokemonImage = document.createElement('img');
  pokemonImage.src = i;
  pokemonImage.height = '500';
  pokemonImage.width = '300';
  document.body.appendChild(pokemonImage);
});
console.log(makeHtmlLists);